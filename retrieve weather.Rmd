...
weather_retrieve <- function( sites, 
     ...
     if ( !("weather.filename" %in% names(sites)) )
          stop("Please first plot sites to weather grids using \"gage.place.weather.grid\" ")
     

     #allow function to take sites argument as *either* data frame or spatial data frame
     if ( attributes(class(sites))$package == "sp" )
          sites <- sites@data
     ...
     #change to look this up in cache/data/weather_set directory
     weather_set_url <- "ftp://192.12.137.7/pub/dcp/archive/OBS/livneh2014.1_16deg/ascii/daily/"
...
#    template_w_date <- some function that loads template of dates by specifying the weather_set
#    template_w_cols <- some function that loads template of columns by specifying the weather_set
     ...
     load( file.path( "C:/ALR/Models/cache/data/weather_sets", weather_set, "weather_grid_coords.rdata"), verbose=T )
     ...
     #
     cat(paste( "Begin processing", nrow(selected.weather.files), 
     ...
     for (i in 1:10)  {
          ...
          for (j in periods ) { 
               ...
               if ( j=="month" ) 
                    x_period <- x_period %>% filter( !is.na(n_days_lag2) )
               ...
               if ( j=="wateryear" ) 
                    x_period <- x_period %>% filter( n_days >= 365 )
...
weather_raw_calc <- function( x, centroid ) {
     ...
     x[,"precip_sum_3"] <- rollsum( x$precip_e, k=3, align="right", fill=NA )
     x[,"precip_sum_5"] <- rollsum( x$precip_e, k=5, align="right", fill=NA )
...
weather_summarize <- function( x_calc, j ) { 
     ...
     cols_max <- c( "precip_e", "precip_raw", "precip_sum_3", "precip_sum_5", "swe_doy"  )
     cols_min <- c()
     ...
     x_summary <- x_calc %>% 
               ...
               summarize( n_days = n() ) #count the number of records in that period
...
weather_lag <- function( y,
                         lag1_cols=c( 
                              "n_days","precip_e_total","pet_total","gdd_total","swe_day_total",
                              "frozen_day_total","tmax_mean","tmin_mean" ),
                         lag2_cols=c( 
                              "n_days","precip_e_total","pet_total",
